...
pub export fn make_parser() void {
    var i: c_int = undefined;
    parser = @as([*c][*c]action, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nstates))) *% @sizeOf([*c]action)))));
    {
        i = 0;
        while (i < nstates) : (i += 1) {
            (blk: {
...
pub export fn free_parser() void {
    var i: c_int = undefined;
    {
        i = 0;
        while (i < nstates) : (i += 1) {
...
pub var SRcount: c_int = @import("std").mem.zeroes(c_int);
pub var RRcount: c_int = @import("std").mem.zeroes(c_int);
pub export fn parse_actions(arg_stateno: c_int) [*c]action {
    var stateno = arg_stateno;
    var actions: [*c]action = undefined;
    actions = get_shifts(stateno);
    actions = add_reductions(stateno, actions);
    return actions;
}
pub export fn get_shifts(arg_stateno: c_int) [*c]action {
    var stateno = arg_stateno;
    var actions: [*c]action = undefined;
    var temp: [*c]action = undefined;
    var sp: [*c]shifts = undefined;
    var tto_state: [*c]c_short = undefined;
    var i: c_int = undefined;
    ...
    if (sp != null) {
        tto_state = @as([*c]c_short, @ptrCast(@alignCast(&sp.*.shift)));
        {
            i = @as(c_int, @bitCast(@as(c_int, sp.*.nshifts))) - @as(c_int, 1);
            while (i >= @as(c_int, 0)) : (i -= 1) {
                k = @as(c_int, @bitCast(@as(c_int, (blk: {
                    const tmp = i;
                    if (tmp >= 0) break :blk tto_state + @as(usize, @intCast(tmp)) else break :blk tto_state - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*)));
                ...
                if (symbol < start_symbol) {
                    temp = @as([*c]action, @ptrCast(@alignCast(allocate(@sizeOf(action)))));
                    temp.*.next = actions;
                    temp.*.symbol = @as(c_short, @bitCast(@as(c_short, @truncate(symbol))));
                    temp.*.number = @as(c_short, @bitCast(@as(c_short, @truncate(k))));
                    temp.*.prec = (blk: {
                        const tmp = symbol;
...
pub export fn add_reductions(arg_stateno: c_int, arg_actions: [*c]action) [*c]action {
    var stateno = arg_stateno;
    var actions = arg_actions;
    var i: c_int = undefined;
    var j: c_int = undefined;
    var m: c_int = undefined;
    var n: c_int = undefined;
    var ruleno: c_int = undefined;
    var tokensetsize: c_int = undefined;
    ...
    }).*)));
    {
        i = m;
        while (i < n) : (i += 1) {
            ruleno = @as(c_int, @bitCast(@as(c_int, (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk LAruleno + @as(usize, @intCast(tmp)) else break :blk LAruleno - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*)));
            rowp = LA + @as(usize, @bitCast(@as(isize, @intCast(i * tokensetsize))));
            {
                j = ntokens - @as(c_int, 1);
                while (j >= @as(c_int, 0)) : (j -= 1) {
                    if ((((blk: {
                        const tmp = j >> @intCast(5);
                        if (tmp >= 0) break :blk rowp + @as(usize, @intCast(tmp)) else break :blk rowp - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                    }).* >> @intCast(j & @as(c_int, 31))) & @as(c_uint, @bitCast(@as(c_int, 1)))) != 0) {
...
pub export fn add_reduce(arg_actions: [*c]action, arg_ruleno: c_int, arg_symbol: c_int) [*c]action {
    var actions = arg_actions;
    var ruleno = arg_ruleno;
    var symbol = arg_symbol;
    var temp: [*c]action = undefined;
    var prev: [*c]action = undefined;
    var next: [*c]action = undefined;
    prev = null;
    {
        next = actions;
        while ((next != null) and (@as(c_int, @bitCast(@as(c_int, next.*.symbol))) < symbol)) : (next = next.*.next) {
            prev = next;
        }
    }
    while (((next != null) and (@as(c_int, @bitCast(@as(c_int, next.*.symbol))) == symbol)) and (@as(c_int, @bitCast(@as(c_uint, next.*.action_code))) == @as(c_int, 1))) {
        prev = next;
        next = next.*.next;
    }
    while ((((next != null) and (@as(c_int, @bitCast(@as(c_int, next.*.symbol))) == symbol)) and (@as(c_int, @bitCast(@as(c_uint, next.*.action_code))) == @as(c_int, 2))) and (@as(c_int, @bitCast(@as(c_int, next.*.number))) < ruleno)) {
        prev = next;
        next = next.*.next;
    }
    temp = @as([*c]action, @ptrCast(@alignCast(allocate(@sizeOf(action)))));
    temp.*.next = next;
    temp.*.symbol = @as(c_short, @bitCast(@as(c_short, @truncate(symbol))));
    temp.*.number = @as(c_short, @bitCast(@as(c_short, @truncate(ruleno))));
    temp.*.prec = (blk: {
        const tmp = ruleno;
...
pub export fn sole_reduction(arg_stateno: c_int) c_short {
    var stateno = arg_stateno;
    var count: c_int = undefined;
    var ruleno: c_short = undefined;
    ...
    ruleno = 0;
    {
        ...
        while (p != null) : (p = p.*.next) {
            if ((@as(c_int, @bitCast(@as(c_uint, p.*.action_code))) == @as(c_int, 1)) and (@as(c_int, @bitCast(@as(c_uint, p.*.suppressed))) == @as(c_int, 0))) return @as(c_short, @bitCast(@as(c_short, @truncate(@as(c_int, 0))))) else if ((@as(c_int, @bitCast(@as(c_uint, p.*.action_code))) == @as(c_int, 2)) and (@as(c_int, @bitCast(@as(c_uint, p.*.suppressed))) == @as(c_int, 0))) {
                if ((@as(c_int, @bitCast(@as(c_int, ruleno))) > @as(c_int, 0)) and (@as(c_int, @bitCast(@as(c_int, p.*.number))) != @as(c_int, @bitCast(@as(c_int, ruleno))))) return @as(c_short, @bitCast(@as(c_short, @truncate(@as(c_int, 0)))));
                if (@as(c_int, @bitCast(@as(c_int, p.*.symbol))) != @as(c_int, 1)) {
                    count += 1;
                }
                ruleno = p.*.number;
            }
    ...
    if (count == @as(c_int, 0)) return @as(c_short, @bitCast(@as(c_short, @truncate(@as(c_int, 0)))));
    return ruleno;
}
pub export fn free_action_row(arg_p: [*c]action) void {
    var p = arg_p;
    var q: [*c]action = undefined;
    while (p != null) {
        q = p.*.next;
...
pub export fn find_final_state() void {
    var goal: c_int = undefined;
    var i: c_int = undefined;
    var tto_state: [*c]c_short = undefined;
    var p: [*c]shifts = undefined;
    p = shift_table[@as(c_uint, @intCast(@as(c_int, 0)))];
    tto_state = @as([*c]c_short, @ptrCast(@alignCast(&p.*.shift)));
    goal = @as(c_int, @bitCast(@as(c_int, ritem[@as(c_uint, @intCast(@as(c_int, 1)))])));
    {
        i = @as(c_int, @bitCast(@as(c_int, p.*.nshifts))) - @as(c_int, 1);
        while (i >= @as(c_int, 0)) : (i -= 1) {
            final_state = (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk tto_state + @as(usize, @intCast(tmp)) else break :blk tto_state - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            ...
            if (@as(c_int, @bitCast(@as(c_int, (blk: {
                const tmp = final_state;
                if (tmp >= 0) break :blk accessing_symbol + @as(usize, @intCast(tmp)) else break :blk accessing_symbol - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*))) == goal) break;
        }
    }
}
pub export fn unused_rules() void {
    var i: c_int = undefined;
    var p: [*c]action = undefined;
    rules_used = @as([*c]c_short, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, nrules))), @sizeOf(c_short)))));
    if (rules_used == @as([*c]c_short, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    {
        i = 0;
        while (i < nstates) : (i += 1) {
            {
                ...
                while (p != null) : (p = p.*.next) {
                    if ((@as(c_int, @bitCast(@as(c_uint, p.*.action_code))) == @as(c_int, 2)) and (@as(c_int, @bitCast(@as(c_uint, p.*.suppressed))) == @as(c_int, 0))) {
                        (blk: {
                            const tmp = p.*.number;
                            if (tmp >= 0) break :blk rules_used + @as(usize, @intCast(tmp)) else break :blk rules_used - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                        }).* = 1;
                    }
                }
            }
    ...
    if (nunused != 0) {
        if (@as(c_int, @bitCast(@as(c_int, nunused))) == @as(c_int, 1)) {
            _ = fprintf(stderr, "%s: 1 rule never reduced\n", __progname);
        } else {
            _ = fprintf(stderr, "%s: %d rules never reduced\n", __progname, @as(c_int, @bitCast(@as(c_int, nunused))));
        }
    }
}
pub export fn remove_conflicts() void {
    var i: c_int = undefined;
    var symbol: c_int = undefined;
    var p: [*c]action = undefined;
    var pref: [*c]action = null;
    SRtotal = 0;
    RRtotal = 0;
    SRconflicts = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nstates))) *% @sizeOf(c_short)))));
    RRconflicts = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nstates))) *% @sizeOf(c_short)))));
    {
        i = 0;
        while (i < nstates) : (i += 1) {
            SRcount = 0;
            ...
            symbol = -@as(c_int, 1);
            {
                ...
                while (p != null) : (p = p.*.next) {
                    if (@as(c_int, @bitCast(@as(c_int, p.*.symbol))) != symbol) {
                        pref = p;
                        symbol = @as(c_int, @bitCast(@as(c_int, p.*.symbol)));
                    } else if ((i == @as(c_int, @bitCast(@as(c_int, final_state)))) and (symbol == @as(c_int, 0))) {
                        SRcount += 1;
                        p.*.suppressed = 1;
                    } else if (@as(c_int, @bitCast(@as(c_uint, pref.*.action_code))) == @as(c_int, 1)) {
                        if ((@as(c_int, @bitCast(@as(c_int, pref.*.prec))) > @as(c_int, 0)) and (@as(c_int, @bitCast(@as(c_int, p.*.prec))) > @as(c_int, 0))) {
                            if (@as(c_int, @bitCast(@as(c_int, pref.*.prec))) < @as(c_int, @bitCast(@as(c_int, p.*.prec)))) {
                                pref.*.suppressed = 2;
                                pref = p;
                            } else if (@as(c_int, @bitCast(@as(c_int, pref.*.prec))) > @as(c_int, @bitCast(@as(c_int, p.*.prec)))) {
                                p.*.suppressed = 2;
                            } else if (@as(c_int, @bitCast(@as(c_uint, pref.*.assoc))) == @as(c_int, 1)) {
                                pref.*.suppressed = 2;
                                pref = p;
                            } else if (@as(c_int, @bitCast(@as(c_uint, pref.*.assoc))) == @as(c_int, 2)) {
                                p.*.suppressed = 2;
                            } else {
                                pref.*.suppressed = 2;
                                p.*.suppressed = 2;
            ...
            (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk SRconflicts + @as(usize, @intCast(tmp)) else break :blk SRconflicts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(c_short, @bitCast(@as(c_short, @truncate(SRcount))));
            (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk RRconflicts + @as(usize, @intCast(tmp)) else break :blk RRconflicts - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(c_short, @bitCast(@as(c_short, @truncate(RRcount))));
        }
    }
}
pub export fn total_conflicts() void {
    if ((SRtotal != SRexpect) or (RRtotal != 0)) {
        if (SRtotal == @as(c_int, 1)) {
            _ = fprintf(stderr, "%s: %s finds 1 shift/reduce conflict\n", input_file_name, __progname);
        } else if (SRtotal > @as(c_int, 1)) {
            _ = fprintf(stderr, "%s: %s finds %d shift/reduce conflicts\n", input_file_name, __progname, SRtotal);
        }
    }
    if (RRtotal == @as(c_int, 1)) {
        _ = fprintf(stderr, "%s: %s finds 1 reduce/reduce conflict\n", input_file_name, __progname);
    } else if (RRtotal > @as(c_int, 1)) {
        _ = fprintf(stderr, "%s: %s finds %d reduce/reduce conflicts\n", input_file_name, __progname, RRtotal);
    }
}
pub export fn defreds() void {
    var i: c_int = undefined;
    defred = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nstates))) *% @sizeOf(c_short)))));
    {
        i = 0;
        while (i < nstates) : (i += 1) {
            (blk: {
