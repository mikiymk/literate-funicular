...
pub export fn write_section(arg_section: [*c][*c]u8) void {
    var section = arg_section;
    ...
    var s: [*c]u8 = undefined;
    {
        ...
        while ((blk: {
            const tmp = (blk_1: {
                const tmp_2 = i;
                if (tmp_2 >= 0) break :blk_1 section + @as(usize, @intCast(tmp_2)) else break :blk_1 section - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
            }).*;
            s = tmp;
            break :blk tmp;
        }) != null) : (i += 1) {
