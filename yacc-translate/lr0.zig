...
pub export fn lr0() void {
    set_derives();
    set_nullable();
    generate_states();
}
pub export fn free_nullable() void {
    free(@as(?*anyopaque, @ptrCast(nullable)));
}
pub export fn free_derives() void {
    ...
    free(@as(?*anyopaque, @ptrCast(derives)));
}
...
pub extern var itemset: [*c]c_short;
pub extern var itemsetend: [*c]c_short;
pub extern var ruleset: [*c]c_uint;
pub export fn get_state(arg_symbol: c_int) c_short {
    var symbol = arg_symbol;
    var n: c_int = undefined;
    var found: c_int = undefined;
    var key: c_int = undefined;
    var isp1: [*c]c_short = undefined;
    var isp2: [*c]c_short = undefined;
    var iend: [*c]c_short = undefined;
    var sp: [*c]core = undefined;
    isp1 = (blk: {
        const tmp = symbol;
        if (tmp >= 0) break :blk kernel_base + @as(usize, @intCast(tmp)) else break :blk kernel_base - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    iend = (blk: {
        const tmp = symbol;
        if (tmp >= 0) break :blk kernel_end + @as(usize, @intCast(tmp)) else break :blk kernel_end - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    n = @as(c_int, @bitCast(@as(c_int, @truncate(@divExact(@as(c_long, @bitCast(@intFromPtr(iend) -% @intFromPtr(isp1))), @sizeOf(c_short))))));
    key = @as(c_int, @bitCast(@as(c_int, isp1.*)));
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if ((@as(c_int, 0) <= key) and (key < nitems)) {} else {
                __assert_fail("0 <= key && key < nitems", "yacc/lr0.c", @as(c_uint, @bitCast(@as(c_int, 203))), "short get_state(int)");
            };
        };
    };
    sp = (blk: {
        const tmp = key;
        if (tmp >= 0) break :blk state_set + @as(usize, @intCast(tmp)) else break :blk state_set - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    if (sp != null) {
        found = 0;
        while (!(found != 0)) {
            if (@as(c_int, @bitCast(@as(c_int, sp.*.nitems))) == n) {
                found = 1;
                isp1 = (blk: {
                    const tmp = symbol;
                    if (tmp >= 0) break :blk kernel_base + @as(usize, @intCast(tmp)) else break :blk kernel_base - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).*;
                isp2 = @as([*c]c_short, @ptrCast(@alignCast(&sp.*.items)));
                while ((found != 0) and (isp1 < iend)) {
...
pub export fn new_state(arg_symbol: c_int) [*c]core {
    var symbol = arg_symbol;
    var n: c_int = undefined;
    var p: [*c]core = undefined;
    var isp1: [*c]c_short = undefined;
    var isp2: [*c]c_short = undefined;
    var iend: [*c]c_short = undefined;
    if (nstates >= @as(c_int, 32767)) {
        fatal(@as([*c]u8, @ptrCast(@volatileCast(@constCast("too many states")))));
    }
    isp1 = (blk: {
        const tmp = symbol;
        if (tmp >= 0) break :blk kernel_base + @as(usize, @intCast(tmp)) else break :blk kernel_base - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    ...
    n = @as(c_int, @bitCast(@as(c_int, @truncate(@divExact(@as(c_long, @bitCast(@intFromPtr(iend) -% @intFromPtr(isp1))), @sizeOf(c_short))))));
    p = @as([*c]core, @ptrCast(@alignCast(allocate(@sizeOf(core) +% (@as(c_ulong, @bitCast(@as(c_long, n - @as(c_int, 1)))) *% @sizeOf(c_short))))));
    p.*.accessing_symbol = @as(c_short, @bitCast(@as(c_short, @truncate(symbol))));
    p.*.number = @as(c_short, @bitCast(@as(c_short, @truncate(nstates))));
    p.*.nitems = @as(c_short, @bitCast(@as(c_short, @truncate(n))));
    isp2 = @as([*c]c_short, @ptrCast(@alignCast(&p.*.items)));
    while (isp1 < iend) {
        (blk: {
            const ref = &isp2;
            const tmp = ref.*;
...
pub export fn allocate_itemsets() void {
    ...
    var symbol_count: [*c]c_short = undefined;
    count = 0;
    symbol_count = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf(c_short)))));
    item_end = ritem + @as(usize, @bitCast(@as(isize, @intCast(nitems))));
    {
        itemp = ritem;
        while (itemp < item_end) : (itemp += 1) {
            symbol = @as(c_int, @bitCast(@as(c_int, itemp.*)));
            if (symbol >= @as(c_int, 0)) {
                count += 1;
                (blk: {
                    const tmp = symbol;
                    if (tmp >= 0) break :blk symbol_count + @as(usize, @intCast(tmp)) else break :blk symbol_count - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).* += 1;
            }
        }
    }
    kernel_base = @as([*c][*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf([*c]c_short)))));
    kernel_items = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, count))) *% @sizeOf(c_short)))));
    count = 0;
    max = 0;
    {
        i = 0;
        while (i < nsyms) : (i += 1) {
            (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk kernel_base + @as(usize, @intCast(tmp)) else break :blk kernel_base - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = kernel_items + @as(usize, @bitCast(@as(isize, @intCast(count))));
            count += @as(c_int, @bitCast(@as(c_int, (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk symbol_count + @as(usize, @intCast(tmp)) else break :blk symbol_count - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*)));
    ...
    kernel_end = @as([*c][*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf([*c]c_short)))));
}
pub export fn allocate_storage() void {
    allocate_itemsets();
    shiftset = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf(c_short)))));
    redset = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nrules + @as(c_int, 1)))) *% @sizeOf(c_short)))));
    state_set = @as([*c][*c]core, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nitems))) *% @sizeOf([*c]core)))));
}
pub export fn append_states() void {
    var i: c_int = undefined;
    ...
    var symbol: c_int = undefined;
    {
        ...
        while (i < nshifts) : (i += 1) {
            symbol = @as(c_int, @bitCast(@as(c_int, (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk shift_symbol + @as(usize, @intCast(tmp)) else break :blk shift_symbol - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*)));
            j = i;
            ...
            (blk: {
                const tmp = j;
                if (tmp >= 0) break :blk shift_symbol + @as(usize, @intCast(tmp)) else break :blk shift_symbol - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = @as(c_short, @bitCast(@as(c_short, @truncate(symbol))));
        }
    }
    {
        i = 0;
        while (i < nshifts) : (i += 1) {
            ...
            (blk: {
                const tmp = i;
                if (tmp >= 0) break :blk shiftset + @as(usize, @intCast(tmp)) else break :blk shiftset - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = get_state(symbol);
        }
    }
}
pub export fn free_storage() void {
    free(@as(?*anyopaque, @ptrCast(shift_symbol)));
    free(@as(?*anyopaque, @ptrCast(redset)));
    free(@as(?*anyopaque, @ptrCast(shiftset)));
    free(@as(?*anyopaque, @ptrCast(kernel_base)));
    free(@as(?*anyopaque, @ptrCast(kernel_end)));
    free(@as(?*anyopaque, @ptrCast(kernel_items)));
    free(@as(?*anyopaque, @ptrCast(state_set)));
}
pub export fn generate_states() void {
    allocate_storage();
    itemset = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nitems))) *% @sizeOf(c_short)))));
    ruleset = @as([*c]c_uint, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, @divTrunc(nrules + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32))))) *% @sizeOf(c_uint)))));
    set_first_derives();
    initialize_states();
    while (this_state != null) {
        closure(@as([*c]c_short, @ptrCast(@alignCast(&this_state.*.items))), @as(c_int, @bitCast(@as(c_int, this_state.*.nitems))));
        save_reductions();
        new_itemsets();
        append_states();
        if (nshifts > @as(c_int, 0)) {
            save_shifts();
        }
...
pub export fn initialize_states() void {
    var i: c_int = undefined;
    var start_derives: [*c]c_short = undefined;
    var p: [*c]core = undefined;
    start_derives = (blk: {
        const tmp = start_symbol;
        if (tmp >= 0) break :blk derives + @as(usize, @intCast(tmp)) else break :blk derives - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).*;
    {
        ...
        while (@as(c_int, @bitCast(@as(c_int, (blk: {
            const tmp = i;
            if (tmp >= 0) break :blk start_derives + @as(usize, @intCast(tmp)) else break :blk start_derives - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
        }).*))) >= @as(c_int, 0)) : (i += 1) continue;
    }
    p = @as([*c]core, @ptrCast(@alignCast(malloc(@sizeOf(core) +% (@as(c_ulong, @bitCast(@as(c_long, i))) *% @sizeOf(c_short))))));
    if (p == @as([*c]core, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    p.*.next = null;
    p.*.link = null;
    ...
    p.*.nitems = @as(c_short, @bitCast(@as(c_short, @truncate(i))));
    {
        i = 0;
        while (@as(c_int, @bitCast(@as(c_int, (blk: {
            const tmp = i;
            ...
            p.*.items[@as(c_uint, @intCast(i))] = (blk: {
                const tmp = (blk_1: {
                    const tmp_2 = i;
                    if (tmp_2 >= 0) break :blk_1 start_derives + @as(usize, @intCast(tmp_2)) else break :blk_1 start_derives - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
                }).*;
...
pub export fn new_itemsets() void {
    var i: c_int = undefined;
    var shiftcount: c_int = undefined;
    var isp: [*c]c_short = undefined;
    var ksp: [*c]c_short = undefined;
    var symbol: c_int = undefined;
    _ = memset(@as(?*anyopaque, @ptrCast(kernel_end)), @as(c_int, 0), @as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf([*c]c_short));
    shiftcount = 0;
    isp = itemset;
    while (isp < itemsetend) {
        ...
        if (symbol > @as(c_int, 0)) {
            ...
            (blk: {
                ...
                ref.* += 1;
                break :blk tmp;
            }).* = @as(c_short, @bitCast(@as(c_short, @truncate(i + @as(c_int, 1)))));
            (blk: {
                const tmp = symbol;
                if (tmp >= 0) break :blk kernel_end + @as(usize, @intCast(tmp)) else break :blk kernel_end - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = ksp;
...
pub export fn save_shifts() void {
    var p: [*c]shifts = undefined;
    var sp1: [*c]c_short = undefined;
    var sp2: [*c]c_short = undefined;
    var send: [*c]c_short = undefined;
    p = @as([*c]shifts, @ptrCast(@alignCast(allocate(@sizeOf(shifts) +% (@as(c_ulong, @bitCast(@as(c_long, nshifts - @as(c_int, 1)))) *% @sizeOf(c_short))))));
    p.*.number = this_state.*.number;
    p.*.nshifts = @as(c_short, @bitCast(@as(c_short, @truncate(nshifts))));
    sp1 = shiftset;
    sp2 = @as([*c]c_short, @ptrCast(@alignCast(&p.*.shift)));
    send = shiftset + @as(usize, @bitCast(@as(isize, @intCast(nshifts))));
    while (sp1 < send) {
        (blk: {
            const ref = &sp2;
            const tmp = ref.*;
...
pub export fn save_reductions() void {
    var isp: [*c]c_short = undefined;
    var rp1: [*c]c_short = undefined;
    var rp2: [*c]c_short = undefined;
    var item: c_int = undefined;
    var count: c_int = undefined;
    var p: [*c]reductions = undefined;
    var rend: [*c]c_short = undefined;
    count = 0;
    {
        isp = itemset;
        while (isp < itemsetend) : (isp += 1) {
            item = @as(c_int, @bitCast(@as(c_int, (blk: {
                const tmp = isp.*;
                if (tmp >= 0) break :blk ritem + @as(usize, @intCast(tmp)) else break :blk ritem - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).*)));
            if (item < @as(c_int, 0)) {
                (blk: {
                    ...
                    if (tmp >= 0) break :blk redset + @as(usize, @intCast(tmp)) else break :blk redset - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).* = @as(c_short, @bitCast(@as(c_short, @truncate(-item))));
            }
        }
    }
    if (count != 0) {
        p = @as([*c]reductions, @ptrCast(@alignCast(allocate(@sizeOf(reductions) +% (@as(c_ulong, @bitCast(@as(c_long, count - @as(c_int, 1)))) *% @sizeOf(c_short))))));
        p.*.number = this_state.*.number;
        p.*.nreds = @as(c_short, @bitCast(@as(c_short, @truncate(count))));
        rp1 = redset;
        rp2 = @as([*c]c_short, @ptrCast(@alignCast(&p.*.rules)));
        rend = rp1 + @as(usize, @bitCast(@as(isize, @intCast(count))));
        while (rp1 < rend) {
            (blk: {
                const ref = &rp2;
                const tmp = ref.*;
...
pub export fn set_derives() void {
    var i: c_int = undefined;
    var k: c_int = undefined;
    var lhs: c_int = undefined;
    var rules: [*c]c_short = undefined;
    derives = @as([*c][*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nsyms))) *% @sizeOf([*c]c_short)))));
    rules = @as([*c]c_short, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nvars + nrules))) *% @sizeOf(c_short)))));
    k = 0;
    {
        lhs = start_symbol;
        while (lhs < nsyms) : (lhs += 1) {
            (blk: {
                const tmp = lhs;
                if (tmp >= 0) break :blk derives + @as(usize, @intCast(tmp)) else break :blk derives - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
            }).* = rules + @as(usize, @bitCast(@as(isize, @intCast(k))));
            {
                i = 0;
                while (i < nrules) : (i += 1) {
                    if (@as(c_int, @bitCast(@as(c_int, (blk: {
                        ...
                        (blk: {
                            const tmp = k;
                            if (tmp >= 0) break :blk rules + @as(usize, @intCast(tmp)) else break :blk rules - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
...
pub extern fn print_derives() void;
pub export fn set_nullable() void {
    var i: c_int = undefined;
    var j: c_int = undefined;
    var empty: c_int = undefined;
    var done: c_int = undefined;
    nullable = @as([*c]u8, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), @as(c_ulong, @bitCast(@as(c_long, nsyms)))))));
    if (nullable == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    done = 0;
    while (!(done != 0)) {
...
pub var state_set: [*c][*c]core = @import("std").mem.zeroes([*c][*c]core);
pub var this_state: [*c]core = @import("std").mem.zeroes([*c]core);
pub var last_state: [*c]core = @import("std").mem.zeroes([*c]core);
pub var last_shift: [*c]shifts = @import("std").mem.zeroes([*c]shifts);
pub var last_reduction: [*c]reductions = @import("std").mem.zeroes([*c]reductions);
pub var nshifts: c_int = @import("std").mem.zeroes(c_int);
pub var shift_symbol: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
pub var redset: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
pub var shiftset: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
pub var kernel_base: [*c][*c]c_short = @import("std").mem.zeroes([*c][*c]c_short);
pub var kernel_end: [*c][*c]c_short = @import("std").mem.zeroes([*c][*c]c_short);
pub var kernel_items: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
