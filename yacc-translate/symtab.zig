...
pub export fn lookup(arg_name: [*c]u8) [*c]bucket {
    var name = arg_name;
    var bp: [*c]bucket = undefined;
    var bpp: [*c][*c]bucket = undefined;
    bpp = symbol_table + @as(usize, @bitCast(@as(isize, @intCast(hash(name)))));
    bp = bpp.*;
    while (bp != null) {
        if (strcmp(name, bp.*.name) == @as(c_int, 0)) return bp;
        bpp = &bp.*.link;
        bp = bpp.*;
    }
    bpp.* = blk: {
        const tmp = make_bucket(name);
        bp = tmp;
        break :blk tmp;
    };
    last_symbol.*.next = bp;
...
pub export fn make_bucket(arg_name: [*c]u8) [*c]bucket {
    var name = arg_name;
    var bp: [*c]bucket = undefined;
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if (name != null) {} else {
                __assert_fail("name", "yacc/symtab.c", @as(c_uint, @bitCast(@as(c_int, 72))), "bucket *make_bucket(char *)");
            };
        };
    };
    bp = @as([*c]bucket, @ptrCast(@alignCast(malloc(@sizeOf(bucket)))));
    if (bp == @as([*c]bucket, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    bp.*.link = null;
    bp.*.next = null;
    bp.*.name = strdup(name);
    if (bp.*.name == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    bp.*.tag = null;
    bp.*.value = @as(c_short, @bitCast(@as(c_short, @truncate(-@as(c_int, 1)))));
    bp.*.index = 0;
    bp.*.prec = 0;
    bp.*.class = 0;
    bp.*.assoc = 0;
...
pub export fn create_symbol_table() void {
    var bp: [*c]bucket = undefined;
    symbol_table = @as([*c][*c]bucket, @ptrCast(@alignCast(calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1024)))), @sizeOf([*c]bucket)))));
    if (symbol_table == @as([*c][*c]bucket, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        no_space();
    }
    bp = make_bucket(@as([*c]u8, @ptrCast(@volatileCast(@constCast("error")))));
    bp.*.index = 1;
    bp.*.class = 1;
    first_symbol = bp;
    last_symbol = bp;
    (blk: {
        const tmp = hash(@as([*c]u8, @ptrCast(@volatileCast(@constCast("error")))));
        if (tmp >= 0) break :blk symbol_table + @as(usize, @intCast(tmp)) else break :blk symbol_table - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
    }).* = bp;
}
pub export fn free_symbol_table() void {
    free(@as(?*anyopaque, @ptrCast(symbol_table)));
    symbol_table = null;
}
pub export fn free_symbols() void {
    var p: [*c]bucket = undefined;
    var q: [*c]bucket = undefined;
    {
        p = first_symbol;
        while (p != null) : (p = q) {
            q = p.*.next;
...
pub export var symbol_table: [*c][*c]bucket = @import("std").mem.zeroes([*c][*c]bucket);
pub export fn hash(arg_name: [*c]u8) c_int {
    var name = arg_name;
    ...
    _ = blk: {
        _ = @sizeOf(c_int);
        break :blk blk_1: {
            break :blk_1 if ((name != null) and (@as(c_int, @bitCast(@as(c_uint, name.*))) != 0)) {} else {
                __assert_fail("name && *name", "yacc/symtab.c", @as(c_uint, @bitCast(@as(c_int, 57))), "int hash(char *)");
            };
        };
    };
    s = name;
    k = @as(c_int, @bitCast(@as(c_uint, s.*)));
    while ((blk: {
        ...
        k = ((@as(c_int, 31) * k) + c) & (@as(c_int, 1024) - @as(c_int, 1));
    }
    return k;
}
...
pub const TABLE_SIZE = @as(c_int, 1024);
