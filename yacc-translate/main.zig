...
pub export fn allocate(arg_n: usize) ?*anyopaque {
    var n = arg_n;
    var v: ?*anyopaque = undefined;
    v = @as(?*anyopaque, @ptrFromInt(@as(c_int, 0)));
    if (n != 0) {
        v = calloc(@as(c_ulong, @bitCast(@as(c_long, @as(c_int, 1)))), n);
        if (!(v != null)) {
            no_space();
        }
    }
...
pub export var file_prefix: [*c]u8 = &(struct {
    var static = "y".*;
}).static;
pub export var explicit_file_name: c_int = @import("std").mem.zeroes(c_int);
pub export fn usage() void {
    _ = fprintf(stderr, "usage: %s [-dlrtv] [-b file_prefix] [-o output_file] [-p symbol_prefix] file\n", __progname);
    exit(@as(c_int, 1));
}
pub export fn getargs(arg_argc: c_int, arg_argv: [*c][*c]u8) void {
    var argc = arg_argc;
    var argv = arg_argv;
    var ch: c_int = undefined;
    while ((blk: {
        const tmp = getopt(argc, argv, "b:dlo:p:rtv");
        ch = tmp;
        break :blk tmp;
    }) != -@as(c_int, 1)) {
        while (true) {
    ...
    if (strcmp(argv.*, "-") == @as(c_int, 0)) {
        input_file = stdin;
    } else {
        input_file_name = argv.*;
    }
}
pub export fn create_file_names() void {
    if (output_file_name == @as([*c]u8, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        if (asprintf(&output_file_name, "%s%s", file_prefix, ".tab.c") == -@as(c_int, 1)) {
            no_space();
        }
    }
    if (rflag != 0) {
        if (asprintf(&code_file_name, "%s%s", file_prefix, ".code.c") == -@as(c_int, 1)) {
            no_space();
        }
    } else {
    ...
    if (dflag != 0) {
        if (explicit_file_name != 0) {
            var suffix: [*c]u8 = undefined;
            defines_file_name = strdup(output_file_name);
            if (defines_file_name == null) {
                no_space();
            }
            if (((blk: {
                const tmp = strrchr(output_file_name, @as(c_int, '.'));
                suffix = tmp;
                break :blk tmp;
            }) != null) and ((((!(strcmp(suffix, ".c") != 0) or !(strcmp(suffix, ".C") != 0)) or !(strcmp(suffix, ".cc") != 0)) or !(strcmp(suffix, ".cxx") != 0)) or !(strcmp(suffix, ".cpp") != 0))) {
                _ = strncpy(defines_file_name, output_file_name, @as(c_ulong, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(suffix) -% @intFromPtr(output_file_name))), @sizeOf(u8)) + @as(c_long, @bitCast(@as(c_long, @as(c_int, 1)))))));
                (blk: {
                    const tmp = @divExact(@as(c_long, @bitCast(@intFromPtr(suffix) -% @intFromPtr(output_file_name))), @sizeOf(u8)) + @as(c_long, @bitCast(@as(c_long, @as(c_int, 1))));
                    if (tmp >= 0) break :blk defines_file_name + @as(usize, @intCast(tmp)) else break :blk defines_file_name - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).* = 'h';
                (blk: {
                    const tmp = @divExact(@as(c_long, @bitCast(@intFromPtr(suffix) -% @intFromPtr(output_file_name))), @sizeOf(u8)) + @as(c_long, @bitCast(@as(c_long, @as(c_int, 2))));
                    if (tmp >= 0) break :blk defines_file_name + @as(usize, @intCast(tmp)) else break :blk defines_file_name - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                }).* = '\x00';
            } else {
                _ = fprintf(stderr, "%s: suffix of output file name %s not recognized, no -d file generated.\n", __progname, output_file_name);
                dflag = 0;
                free(@as(?*anyopaque, @ptrCast(defines_file_name)));
                defines_file_name = null;
            }
        } else {
            if (asprintf(&defines_file_name, "%s%s", file_prefix, ".tab.h") == -@as(c_int, 1)) {
                no_space();
            }
        }
    }
    if (vflag != 0) {
        if (asprintf(&verbose_file_name, "%s%s", file_prefix, ".output") == -@as(c_int, 1)) {
            no_space();
        }
    }
}
pub export fn open_files() void {
    create_file_names();
    if (input_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
    ...
    if (vflag != 0) {
        ...
        if (verbose_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            open_error(verbose_file_name);
        }
    }
    if (dflag != 0) {
        defines_file = fopen(defines_file_name, "w");
        if (defines_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
            open_write_error(defines_file_name);
        }
        union_file = create_temp();
    }
    output_file = fopen(output_file_name, "w");
    if (output_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
        open_error(output_file_name);
    }
    if (rflag != 0) {
        code_file = fopen(code_file_name, "w");
        if (code_file == @as([*c]FILE, @ptrCast(@alignCast(@as(?*anyopaque, @ptrFromInt(@as(c_int, 0))))))) {
...
pub export fn create_temp() [*c]FILE {
...
pub const _PATH_VARDB = "/var/lib/misc/";
