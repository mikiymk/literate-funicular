...
pub export fn reflexive_transitive_closure(arg_R: [*c]c_uint, arg_n: c_int) void {
    var R = arg_R;
    var n = arg_n;
    var rowsize: c_int = undefined;
    var i: c_uint = undefined;
    var rp: [*c]c_uint = undefined;
    var relend: [*c]c_uint = undefined;
    transitive_closure(R, n);
    rowsize = @divTrunc(n + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    relend = R + @as(usize, @bitCast(@as(isize, @intCast(n * rowsize))));
    i = 0;
    rp = R;
    while (rp < relend) {
        rp.* |= @as(c_uint, @bitCast(@as(c_int, 1) << @intCast(i)));
        if ((blk: {
            const ref = &i;
            ref.* +%= 1;
            break :blk ref.*;
        ...
        rp += @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
    }
}
...
pub export fn transitive_closure(arg_R: [*c]c_uint, arg_n: c_int) void {
    var R = arg_R;
    var n = arg_n;
    var rowsize: c_int = undefined;
    var i: c_uint = undefined;
    var rowj: [*c]c_uint = undefined;
    var rp: [*c]c_uint = undefined;
    var rend: [*c]c_uint = undefined;
    var ccol: [*c]c_uint = undefined;
    var relend: [*c]c_uint = undefined;
    var cword: [*c]c_uint = undefined;
    var rowi: [*c]c_uint = undefined;
    rowsize = @divTrunc(n + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    relend = R + @as(usize, @bitCast(@as(isize, @intCast(n * rowsize))));
    cword = R;
    i = 0;
    ...
    while (rowi < relend) {
        ...
        rowj = R;
        while (rowj < relend) {
            if ((ccol.* & @as(c_uint, @bitCast(@as(c_int, 1) << @intCast(i)))) != 0) {
                rp = rowi;
                rend = rowj + @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
                while (rowj < rend) {
                    (blk: {
                        const ref = &rowj;
                        const tmp = ref.*;
                ...
                rowj += @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
            }
            ccol += @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
        }
        if ((blk: {
            const ref = &i;
            ref.* +%= 1;
        ...
        rowi += @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
    }
}
