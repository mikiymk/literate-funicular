...
pub export fn strlcpy(arg_dst: [*c]u8, arg_src: [*c]const u8, arg_dsize: c_ulong) c_ulong {
    var dst = arg_dst;
    var src = arg_src;
    var dsize = arg_dsize;
    var osrc: [*c]const u8 = src;
    var nleft: usize = dsize;
    if (nleft != @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) {
        while ((blk: {
            const ref = &nleft;
            ref.* -%= 1;
            break :blk ref.*;
    ...
    if (nleft == @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) {
        if (dsize != @as(usize, @bitCast(@as(c_long, @as(c_int, 0))))) {
            dst.* = '\x00';
        }
        while ((blk: {
            const ref = &src;
    ...
    return @as(c_ulong, @bitCast(@divExact(@as(c_long, @bitCast(@intFromPtr(src) -% @intFromPtr(osrc))), @sizeOf(u8)) - @as(c_long, @bitCast(@as(c_long, @as(c_int, 1))))));
}
