...
pub export fn set_first_derives() void {
    var rrow: [*c]c_uint = undefined;
    var vrow: [*c]c_uint = undefined;
    var k: c_uint = undefined;
    var cword: c_uint = 0;
    var i: c_int = undefined;
    var j: c_int = undefined;
    var rule: c_int = undefined;
    var rulesetsize: c_int = undefined;
    var varsetsize: c_int = undefined;
    var rp: [*c]c_short = undefined;
    rulesetsize = @divTrunc(nrules + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    varsetsize = @divTrunc(nvars + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    first_derives = @as([*c]c_uint, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nvars * rulesetsize))) *% @sizeOf(c_uint))))) - @as(usize, @bitCast(@as(isize, @intCast(ntokens * rulesetsize))));
    set_EFF();
    rrow = first_derives + @as(usize, @bitCast(@as(isize, @intCast(ntokens * rulesetsize))));
    {
        i = start_symbol;
        while (i < nsyms) : (i += 1) {
            vrow = EFF + @as(usize, @bitCast(@as(isize, @intCast((i - ntokens) * varsetsize))));
            k = 32;
            {
                j = start_symbol;
                while (j < nsyms) : (_ = blk: {
                    ...
                    if (k >= @as(c_uint, @bitCast(@as(c_int, 32)))) {
                        cword = (blk: {
                            const ref = &vrow;
                            const tmp = ref.*;
                            ref.* += 1;
                    ...
                    if ((cword & @as(c_uint, @bitCast(@as(c_int, 1) << @intCast(k)))) != 0) {
                        rp = (blk: {
                            const tmp = j;
                        ...
                        while ((blk: {
                            ...
                            _ = blk: {
                                const ref = &(blk_1: {
                                    const tmp = rule >> @intCast(5);
                                    if (tmp >= 0) break :blk_1 rrow + @as(usize, @intCast(tmp)) else break :blk_1 rrow - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                                }).*;
                                ref.* |= @as(c_uint, @bitCast(@as(c_int, 1))) << @intCast(rule & @as(c_int, 31));
                                break :blk ref.*;
                            };
                        }
                    }
            ...
            rrow += @as(usize, @bitCast(@as(isize, @intCast(rulesetsize))));
        }
    }
    free(@as(?*anyopaque, @ptrCast(EFF)));
}
pub export fn closure(arg_nucleus: [*c]c_short, arg_n: c_int) void {
    var nucleus = arg_nucleus;
    var n = arg_n;
    var i: c_uint = undefined;
    var word: c_uint = undefined;
    var csp: [*c]c_short = undefined;
    var csend: [*c]c_short = undefined;
    var dsp: [*c]c_uint = undefined;
    var rsp: [*c]c_uint = undefined;
    var rsend: [*c]c_uint = undefined;
    var rulesetsize: c_int = undefined;
    var ruleno: c_int = undefined;
    var symbol: c_int = undefined;
    var itemno: c_int = undefined;
    rulesetsize = @divTrunc(nrules + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    rsend = ruleset + @as(usize, @bitCast(@as(isize, @intCast(rulesetsize))));
    _ = memset(@as(?*anyopaque, @ptrCast(ruleset)), @as(c_int, 0), @as(c_ulong, @bitCast(@as(c_long, rulesetsize))) *% @sizeOf(c_uint));
    csend = nucleus + @as(usize, @bitCast(@as(isize, @intCast(n))));
    {
        csp = nucleus;
        while (csp < csend) : (csp += 1) {
            ...
            if (symbol >= start_symbol) {
                dsp = first_derives + @as(usize, @bitCast(@as(isize, @intCast(symbol * rulesetsize))));
                rsp = ruleset;
                while (rsp < rsend) {
                    (blk: {
                        const ref = &rsp;
    ...
    {
        rsp = ruleset;
        while (rsp < rsend) : (rsp += 1) {
            word = rsp.*;
            if (word != 0) {
                {
                    i = 0;
                    while (i < @as(c_uint, @bitCast(@as(c_int, 32)))) : (i +%= 1) {
                        if ((word & @as(c_uint, @bitCast(@as(c_int, 1) << @intCast(i)))) != 0) {
                            itemno = @as(c_int, @bitCast(@as(c_int, rrhs[@as(c_uint, @bitCast(ruleno)) +% i])));
                            while ((csp < csend) and (@as(c_int, @bitCast(@as(c_int, csp.*))) < itemno)) {
                                (blk: {
                                    const ref = &itemsetend;
                                    const tmp = ref.*;
                                    ref.* += 1;
                                    break :blk tmp;
                                }).* = (blk: {
                            ...
                            (blk: {
                                ...
                                ref.* += 1;
                                break :blk tmp;
                            }).* = @as(c_short, @bitCast(@as(c_short, @truncate(itemno))));
                            while ((csp < csend) and (@as(c_int, @bitCast(@as(c_int, csp.*))) == itemno)) {
                                csp += 1;
                            }
                        }
...
pub export fn finalize_closure() void {
    free(@as(?*anyopaque, @ptrCast(itemset)));
    free(@as(?*anyopaque, @ptrCast(ruleset)));
    free(@as(?*anyopaque, @ptrCast(first_derives + @as(usize, @bitCast(@as(isize, @intCast(ntokens * @divTrunc(nrules + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32)))))))));
}
...
pub export var itemset: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
pub export var itemsetend: [*c]c_short = @import("std").mem.zeroes([*c]c_short);
pub export var ruleset: [*c]c_uint = @import("std").mem.zeroes([*c]c_uint);
pub var first_derives: [*c]c_uint = @import("std").mem.zeroes([*c]c_uint);
pub var EFF: [*c]c_uint = @import("std").mem.zeroes([*c]c_uint);
pub fn set_EFF() callconv(.C) void {
    var row: [*c]c_uint = undefined;
    var symbol: c_int = undefined;
    var rowsize: c_int = undefined;
    var i: c_int = undefined;
    var rule: c_int = undefined;
    var sp: [*c]c_short = undefined;
    rowsize = @divTrunc(nvars + (@as(c_int, 32) - @as(c_int, 1)), @as(c_int, 32));
    EFF = @as([*c]c_uint, @ptrCast(@alignCast(allocate(@as(c_ulong, @bitCast(@as(c_long, nvars * rowsize))) *% @sizeOf(c_uint)))));
    row = EFF;
    {
        i = start_symbol;
        while (i < nsyms) : (i += 1) {
            ...
            }).*;
            {
                ...
                while (rule > @as(c_int, 0)) : (rule = @as(c_int, @bitCast(@as(c_int, (blk: {
                    const ref = &sp;
                    ref.* += 1;
                    break :blk ref.*;
                }).*)))) {
                    symbol = @as(c_int, @bitCast(@as(c_int, (blk: {
                        const tmp = (blk_1: {
                            const tmp_2 = rule;
                            if (tmp_2 >= 0) break :blk_1 rrhs + @as(usize, @intCast(tmp_2)) else break :blk_1 rrhs - ~@as(usize, @bitCast(@as(isize, @intCast(tmp_2)) +% -1));
                        }).*;
                        if (tmp >= 0) break :blk ritem + @as(usize, @intCast(tmp)) else break :blk ritem - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                    }).*)));
                    if (symbol >= start_symbol) {
                        ...
                        _ = blk: {
                            const ref = &(blk_1: {
                                ...
                                if (tmp >= 0) break :blk_1 row + @as(usize, @intCast(tmp)) else break :blk_1 row - ~@as(usize, @bitCast(@as(isize, @intCast(tmp)) +% -1));
                            }).*;
            ...
            row += @as(usize, @bitCast(@as(isize, @intCast(rowsize))));
        }
    }
